...
const makeScrollButton = ActionButton => ActionButton.extend`
  ...
  position: absolute;
  ${props =>
    props.left ? (props.showModalButton ? `left: ${buttonWidth + 2}px` : `left: 0`) : 'right: 0'};
  &:hover {
    cursor: pointer;
  }
`;
...
export default class TabListComponent extends React.Component<Props, State> {
  ...
  renderTabs(options?: any = {}, isModal?: boolean) {
    const { children, activeIndex, handleEdit, customStyle } = this.props;
    const props = {
      handleTabChange: this.handleTabChangeAndCloseModal.bind(this),
      handleEdit,
      //$FlowFixMe
  ...
  handleTabChangeAndCloseModal(index) {
    this.props.handleTabChange(index);
    if (this.state.modalIsOpen && this.props.autoCloseModal) {
      this.scrollToIndex(this.props.activeIndex, 'left');
  ...
  render() {
    ...
    return (
      <div className="app_tab_bar">
        {ExtraButton && this.state.showArrowButton ? ExtraButton : null}
        <TabList
          ...
          {this.state.showModalButton ? (
            <FoldButton
              innerRef={node => (this.foldNode = node)}
          ...
          <ListInner innerRef={node => (this.listContainer = node)} className={'tablist-inner'}>
            <ListScroll
              innerRef={node => (this.listScroll = node)}
              role="tablist"
              className={'tablist-scroll'}>
              {this.renderTabs()}
              {ExtraButton && !this.state.showArrowButton ? ExtraButton : null}
            </ListScroll>
          </ListInner>
        ...
        {modalIsOpen ? (
          <TabModal
            ...
            className={'tab-modal'}>
            <span className="tab-modal-close" onClick={this.toggleModal.bind(this, false)}><i className="apier api-tab-modal-close" /></span>
