...
type ThemeInner = {
  ...
  styles: ?{
    ...
    link?: ThemeBlock,
    sendRequest?: ThemeBlock,
    toolbarBtn?: ThemeBlock,
    pasterPanel?: ThemeBlock,
    horBtn?: ThemeBlock,
    verBtn?: ThemeBlock,
    sendBtn?: ThemeBlock,
    sendDropdown?: ThemeBlock,
    reactTabs?: ThemeBlock,
    responseHeader?: ThemeBlock,
    noResponseHeader?: ThemeBlock,
    responseTabs? : ThemeBlock,
    responseOverlay? : ThemeBlock
  }
};
...
export async function generateThemeCSS(theme: PluginTheme): Promise<string> {
  ...
  if (renderedTheme.styles) {
    ...
    // Send Request Button
    css += wrapStyles(n, '.send_request', getThemeBlockCSS(styles.sendRequest));

    // Toolbar Button
    css += wrapStyles(n, '.app_header_btn', getThemeBlockCSS(styles.toolbarBtn));

    css += wrapStyles(n, '.paster_panel', getThemeBlockCSS(styles.pasterPanel));

    css += wrapStyles(n, '.horBtn', getThemeBlockCSS(styles.horBtn));
    css += wrapStyles(n, '.verBtn', getThemeBlockCSS(styles.verBtn));

    css += wrapStyles(n, '.send_request', getThemeBlockCSS(styles.sendBtn));
    css += wrapStyles(n, '.send_dropdown', getThemeBlockCSS(styles.sendDropdown));

    css += wrapStyles(n, '.react-tabs', getThemeBlockCSS(styles.reactTabs));

    css += wrapStyles(n, '.response_header', getThemeBlockCSS(styles.responseHeader));
    css += wrapStyles(n, '.no_response', getThemeBlockCSS(styles.noResponseHeader));

    css += wrapStyles(n, '.react-tabs__tab-list', getThemeBlockCSS(styles.responseTabs));
    css += wrapStyles(n, '.response_overlay', getThemeBlockCSS(styles.responseOverlay));
  }
...
function getThemeBlockCSS(block?: ThemeBlock): string {
  ...
  if (block.background) {
    ...
    addVar('color-white', background.white);
    addVar('color-window', background.window);
    addVar('color-gray', background.gray);
    addVar('color-light', background.light);
    addVar('color-silver', background.silver);
    addVar('color-main', background.main);
    addVar('color-line', background.line);
    addVar('color-darkline', background.darkline);
    addVar('color-tab', background.tab);
    addVar('color-tab-border', background.tabborder);
    addVar('color-tab-active', background.tabactive);
    addVar('color-tab-hover', background.tabhover);
    addVar('color-paster', background.paster);
    addVar('color-sel', background.sel);
    addVar('color-saving', background.saving);
    addVar('color-post', background.post);
    addVar('color-get', background.get);
    addVar('color-delete', background.delete);
    addVar('color-patch', background.patch);
    addVar('color-put', background.put);
    addVar('color-others', background.others);
    addVar('btn-hover', background.hover);
    addNewLine();
  }
  ...
  if (block.foreground) {
    ...
    addVar('color-font-tag', foreground.tag);
    addVar('color-font-light', foreground.light);
    addVar('color-font-paster', foreground.paster);
    addVar('color-font-post', foreground.post);
    addVar('color-font-get', foreground.get);
    addVar('color-font-put', foreground.put);
    addVar('color-font-delete', foreground.delete);
    addVar('color-font-patch', foreground.patch);
    addVar('color-font-others', foreground.others);
    addNewLine();
  }
