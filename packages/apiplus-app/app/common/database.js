...
export function getDBFilePath(modelType) {
  // NOTE: Do not EVER change this. EVER!
  const { app } = electron.remote || electron;
  const basePath = app.getPath('userData');
  return fsPath.join(basePath, `apiplus.${modelType}.db`);
}
...
export const compactData = (database.compactData = async function() {
  for (const type of ["Workspace", "WorkspaceMeta", "Request"]) {
    const filePath = getDBFilePath(type);
    const collection = new NeDB(
      Object.assign(
        {
          autoload: true,
...
export const remove = (database.remove = async function<T: BaseModel>(
  ...
  // 如果是删除 workspace，那么只需要提交删除项目的通知，不需要再提交删除 request 等的通知给 sync
...
export async function docUpdate<T: BaseModel>(
  ...
  const doc = await models.initModel(
    ...
    // "Workspace", "RequestGroup", "Request" 不再每次改变都修改改变时间，只有当保存 / 重命名 / 排序 / 删除的时候才改变
    // { modified: ["Request"].includes(originalDoc.type) ? originalDoc.modified : Date.now() },
    { modified: Date.now() },
...
export const duplicate = (database.duplicate = async function<T: BaseModel>(
  ...
  async function next<T: BaseModel>(docToCopy: T, patch: Object): Promise<T> {
    ...
    delete newDoc.showid;
    delete newDoc.members;
    delete newDoc.owner;
