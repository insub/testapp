...
export async function _actuallySend(
  ...
  return new Promise(async resolve => {
    ...
    try {
      ...
      // 修改了 paramters 机制，url query 和 paramters 会同时存在，因此实际发请求的时候不能再在 url 上拼接 parameters
      const url = renderedRequest.url
      ...
      // If we calculated the body within apiplus (ie. not computed by Curl)
      if (requestBody !== null) {
      ...
      if (!hasUserAgentHeader(headers)) {
        setOpt(Curl.option.USERAGENT, `apiplus/${getAppVersion()}`);
      }
...
export async function send(requestId: string, environmentId: string): Promise<ResponsePatch> {
  ...
  if (!hasAddEventListener){
    document.addEventListener('keydown', (e: KeyboardEvent) => {
      ...
      console.warn("hasAddEventListener", lastUserInteraction)
    });
...
// 此处在 workspace 里 import 会导致 document is not defined，移到 send 函数中了
// document.addEventListener('keydown', (e: KeyboardEvent) => {
//   if (e.ctrlKey || e.metaKey || e.altKey) {
//     return;
//   }
