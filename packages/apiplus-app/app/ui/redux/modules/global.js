...
const LOCALSTORAGE_PREFIX = `apiplus::meta`;
...
const COMMAND_CLIENT_LOGIN = 'app/client/login'
...
function pushingReducer(state = false, action) {
  switch (action.type) {
    case 'global/push-start':
      return true;
    case 'global/push-stop':
      return false;
    default:
      return state;
  }
...
function pullingReducer(state = false, action) {
  switch (action.type) {
    case 'global/pull-start':
      return true;
    case 'global/pull-stop':
      return false;
    default:
      return state;
  }
...
export const reducer = combineReducers({
  ...
  isPushing: pushingReducer,
  isPulling: pullingReducer
});
  ...
  return async dispatch => {
    switch (command) {
      ...
      case COMMAND_IMPORT_URI:
        ...
        break;
      case COMMAND_CLIENT_LOGIN:
        showModal(LoginModal, { title: args.title, message: args.message, skey: args.skey });
        break;
    }
  };
}
...
export function exportFile(workspaceId = null) {
  return dispatch => {
    ...
    showModal(SelectModal, {
      ...
      options: [
        {
          name: 'apiplus – Sharable with other apiplus users',
          value: VALUE_JSON
        },
      ...
      onDone: async selectedFormat => {
        ...
        const name = (workspace ? workspace.name : 'Apiplus All').replace(
          / /g,
          '-'
        );
        ...
        if (selectedFormat === VALUE_HAR) {
          ...
          options.filters = [{ name: 'Apiplus Export', extensions: ['json'] }];
        }
