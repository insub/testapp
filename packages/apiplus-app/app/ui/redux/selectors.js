...
export const selectUidWorkspaces = createSelector(
  state => selectEntitiesLists(state).workspaces,
  state => selectEntitiesLists(state).workspaceMetas, 
  ...
  const uidWorkspaces = workspaces.filter(workspace => {
    ...
    return !!(meta && meta.uid == session.getBaseid() || null);
  }).reverse();
  console.warn("uidWorkspaces", uidWorkspaces)
...
export const selectActiveWorkspace = createSelector(
  ...
  selectUidWorkspaces,
  (entities, activeWorkspaceId, uidWorkspaces) => {
    const finalWorkspace = uidWorkspaces.filter(w => w._id === activeWorkspaceId).reverse()[0] || uidWorkspaces[0]
    // console.warn("finalWorkspace --------", finalWorkspace, activeWorkspaceId)
    window.localStorage.setItem(
      `apiplus::meta::activeWorkspaceId`,
      JSON.stringify(finalWorkspace._id)
    );
    // workspaces is Array, entities.workspaces is Hash，此处原版有 bug, 删除时 apiplus::meta::activeWorkspaceId 没有改变
  ...
  (collapsed, requestsAndRequestGroups, activeWorkspace, activeWorkspaceMeta) => {
    ...
    function matchChildren(children, parentNames = []) {
      ...
      for (const child of children) {
        ...
        const { name, method, url } = child.doc;
        const showid = child.doc.showid ? `#${child.doc.showid}` : ''
        const match = fuzzyMatchAll(sidebarFilter, [name, method, url, showid, ...parentNames], {
          splitSpace: true
        });
...
export const selectUidMessages = createSelector(selectEntitiesLists, entities => {
  return entities.messages.filter(m => m.uid === session.getBaseid()).reverse();
});

export const selectRequestMetas = createSelector(selectEntitiesLists, entities => {
  return entities.requestMetas
});
