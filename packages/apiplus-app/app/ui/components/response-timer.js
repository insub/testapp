...
class ResponseTimer extends PureComponent {
  ...
  render() {
    const { handleCancel, loadStartTime, request } = this.props;
    ...
    return (
      <div
        className={classnames('overlay theme--transparent-overlay', 'response_overlay', {
          'overlay--hidden': !show
        })}>
        ...
        <div className={`pad http-method-${request.method}`}>
          <i className="fa fa-refresh fa-spin" />
        </div>
        <div className={`pad http-method-${request.method}`}><span>{request.method} {request.host}{request.url}</span></div>
        <div className="pad">
