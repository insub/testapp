...
class SidebarRequestGroupRow extends PureComponent {
  ...
  render() {
    ...
    let folderIconClass = 'apiplus api-folder';
    let arrowIconClass = 'apiplus api-downarrow';

    folderIconClass += isCollapsed ? '' : '_open';
    folderIconClass += isActive ? '' : '';

    arrowIconClass += isCollapsed ? '' : '_exp';
    arrowIconClass += isActive ? '' : '';
    ...
    const button = connectDragSource(
      connectDropTarget(
        <button onClick={this._handleCollapse} onContextMenu={this._handleShowActions}>
          <div className="sidebar__clickable folder_item">
            <i className={'sidebar__item__arrow fa ' + arrowIconClass} />
        ...
        <ul
          ...
          {!isCollapsed && children.length > 0 ? (
            ...
            <SidebarRequestRow
              ...
              handleDuplicateRequestToTemp={misc.nullFn}
