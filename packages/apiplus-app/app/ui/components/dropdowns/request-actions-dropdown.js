...
class RequestActionsDropdown extends PureComponent {
  ...
  _handleDuplicate() {
    const { request, handleDuplicateRequest, handleDuplicateRequestToTemp } = this.props;
    handleDuplicateRequestToTemp(request);
  }
  ...
  _handleRemove() {
    ...
    this.props.handleTab('delete', request._id).then(() => {
