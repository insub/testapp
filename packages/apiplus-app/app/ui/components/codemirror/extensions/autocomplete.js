...
CodeMirror.defineOption('environmentAutocomplete', null, (cm, options) => {
  ...
  async function completeAfter(cm, fn, showAllOnNoMatch = false) {
    ...
    // let pasterx = document.querySelector('#paster_panel');
    let chtml = `
      <h2>xxxxxxx!</h2>
    `;
    if (!hintsContainer) {
    ...
    hintsContainer.innerHTML = chtml
    // hintsContainer.appendChild(pasterx);
  ...
  cm.addKeyMap({
    ...
    'Ctrl-X': completeForce, // Force autocomplete on hotkey
