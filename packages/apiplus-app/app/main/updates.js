...
async function getUpdateUrl(force: boolean): Promise<string | null> {
  ...
  if (platform === 'win32') {
    ...
    updateUrl = UPDATE_URL_LINUX;
  } else {
    return null;
  }

  const params = [
    { name: 'system', value: platform },
  ...
  if (process.env.APIPLUS_DISABLE_AUTOMATIC_UPDATES) {
    console.log(
      `[updater] Disabled by APIPLUS_DISABLE_AUTOMATIC_UPDATES environment variable`
    );
    return null;
  }
